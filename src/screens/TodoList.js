...
const TodoList = () => {
  ...
  const renderItem = ({ item }) => (
    <TodoCube
      data={item}
      onHandleUpdate={() => updateTodo(item)}
      onHandleDelete={() => deleteTodo(item.id)}
    />
  );
  // todos list ui
  return (
    <SafeAreaView style={styles.bgWrapper}>
      <Header
        title="Todo List"
        subTitle="Let's create your todo list"
        signOut={singOut}
      />
      <Content>
        <FlatList
      ...
      <TextEnter
        inputRef={inputRef}
        onChagne={handleTextChange}
        onHandlePress={addTodo}
        btnText={todoId != null ? "EDIT" : "ADD"}
      />
    </SafeAreaView>
  );
};
